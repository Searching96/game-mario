...
void CMario::SetState(int state)
{
	...
	case MARIO_STATE_RUNNING_RIGHT:
		...
		// Ki?m tra rõ ràng hon cho tr?ng thái nh?y và chuy?n hu?ng
		if (!isOnPlatform)
		{
			if (vx < 0)  // Ðang d?ng im ho?c dang di sang trái trong không trung
			{
	...
	case MARIO_STATE_RUNNING_LEFT:
		...
		if (!isOnPlatform)
		{
			if (vx > 0)  // Ðang d?ng im ho?c dang di sang ph?i trong không trung
			{
				//vx = MARIO_RUNNING_SPEED / 5;
				ax = -MARIO_DECELERATION_X;
			}
